...
public class GangliaWriter extends BaseOutputWriter {
    ...
    public void validateSetup(Query query) throws ValidationException {
		...
		log.debug( "validated ganglia metric -- address: " + host + ":" + port + ", spoofed host: " + spoofedHostname + ", group: " + groupName + ", units: " + units + ", slope:" + slope + ", tmax: " + tmax + ", dmax: " + dmax );
    }

	/**
	 * Determines the spoofed host name to be used when emitting metrics to a gmond process.
	 * Spoofed host names are of the form IP:hostname.
	 * 
	 * @param host the host of the gmond (ganglia monitor) to which we are connecting, not null
	 * @param alias the custom alias supplied, may be null
	 * @return the host name to use when emitting metrics, in the form of IP:hostname
	 */
	public static String getSpoofedHostName( String host, String alias ) {
		// Determine the host name to use as the spoofed host name, this should be of the format IP:hostname
		String spoofed = host;
		if( StringUtils.isNotEmpty( alias ) ) {
			// If the alias was supplied in the appropriate format, use it directly
		...
		// Attempt to find the IP of the given host (this may be an aliased host)
		try {
		...
		// We failed to resolve the spoofed host or our local host, return "x" for the IP
		return "x:" + spoofed;
	}
		...
		throws IOException
	{
		...
		xdr_int(dmax); // dmax, the maximum time (in seconds) to store the metric value, 0 = forever
